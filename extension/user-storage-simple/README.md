...
-------------------------------
Login to the <span>Red Hat build of Keycloak</span> Admin Console and got to the User Federation tab.   You should now see your deployed providers in the add-provider list box.
...
admin console or on the registration pages of <span>Red Hat build of Keycloak</span>, will be created in the properties file you configured.  If you go
...
1. Make sure you have an Red Hat build of Keycloak server running with an admin user in the `master` realm or use the provided docker image. Your <span>Red Hat build of Keycloak</span> should be listening on `http://localhost:8180`. You can archive this by running:
