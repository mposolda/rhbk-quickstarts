...
Summary: Single Page Application protected using the Red Hat build of Keycloak JavaScript Adapter  
...
using Red Hat build of Keycloak. Once authenticated the application shows how to invoke a service secured with Red Hat build of Keycloak.
...
Make sure Red Hat build of Keycloak is [running](#starting-and-configuring-the-Red Hat build of Keycloak-server). At the same time, the `npm` should be stopped, so there is nothing listening on http://localhost:8080 .
